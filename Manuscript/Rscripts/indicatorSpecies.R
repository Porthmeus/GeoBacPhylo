...
for(valName in names(result)){
    ...
    ESVs <- c() #rownames(value[["vennMat"]])[apply(value[["vennMat"]], 1, sum) ==length(cntrsts)]
    if(length(cntrsts) > 1){
        ...
        for(feat in features){
            indicators[[feat]] <- rownames(value[["vennMat"]])[apply(value[["vennMat"]], 1, sum) ==length(cntrsts)]
        }
    }

    barTabs <- list()
    for(con in names(indicators)){
        resultTabs <- c(grep(paste0("^",con), names(value[["results"]]),value = TRUE),
                        grep(paste0("_",con), names(value[["results"]]),value = TRUE))
        barTab <- list()
...
#        p_barplot <- ggplot(barTab[barTab[["log2FoldChange"]]>0,], aes(x=log2FoldChange, y = shortID, fill = PhyClass))+
...
#    } else {
        p_barplot <- ggplot(barTab[barTab[["log2FoldChange"]] > 0,], aes(x=log2FoldChange, y = shortID, fill = PhyClass, color = vs.))+
...
# finally export the tax table as reference for the paper and corresponding fasta for blasting tasks
write.csv(tax, file ="tables/ESV_taxonomy.csv")
fasta <- paste(apply(tax[,c("shortID","rep.seq")], 1, function(x) paste0(">",x[1],"\n",x[2],"\n")), collapse = "\n")
cat(fasta, file = "tables/ESV_16S.fas")
