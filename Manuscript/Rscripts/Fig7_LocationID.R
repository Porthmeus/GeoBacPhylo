...
p_umap <- ggplot(p_data, aes(x=UMAP1, y=UMAP2, color = LocationID)) +
    geom_point(size = 2) +
    theme_bw() +
...
p_pca <- ggplot(p_data, aes(x=PC1, y=PC2, color = LocationID)) +
    geom_point(size = 2) +
...
adn <- adonis2(brayCurt~LocationID, data=testMeta, by="margin")
print(adn)
...
brayCurt_long[,LocationID := c("within","between")[as.integer(meta[Sample1, "LocationID"] != meta[Sample2, "LocationID"])+1]]
brayCurt_long[,LocationID := factor(LocationID, levels = c("within","between"))]
LocationID_long <- paste(meta[brayCurt_long[,Sample1],"LocationID"],meta[brayCurt_long[,Sample2], "LocationID"], sep = "-")
n <- combn(unique(meta[["LocationID"]]),2)
crct <- apply(n,2,paste,collapse="-")
wrng <- apply(n[c(2,1),],2,paste,collapse="-")
for(i in 1:ncol(n)){
    LocationID_long <- gsub(wrng[i],crct[i],LocationID_long)
}
brayCurt_long[,LocationID_long:=LocationID_long]


p_boxDist <- ggplot(data = brayCurt_long, aes(x=LocationID, y=Bray_Curtis)) +
    geom_boxplot() +
    ylab("Distance") +
    theme_bw() +
    xlab("") +
...
alphaDiv[["LocationID"]] <- with(alphaDiv, factor(LocationID, levels = unique(LocationID[order(SiteID)])))

p_alpha <- ggplot(alphaDiv, aes(x=LocationID, y=value, fill = SiteID)) +
    geom_boxplot() +
...
ggsave(p_all,file = "figures/Fig7_LocationID.pdf", width = 8, height= 8)
