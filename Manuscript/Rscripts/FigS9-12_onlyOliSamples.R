...
meta[["Water_body"]] <- with(meta,gsub("Lake","standing",gsub("River","flowing", Water_body)))
...
# use only those samples for oligactis
...
p_umap <- ggplot(p_data, aes(x=UMAP1, y=UMAP2, color = Water_body)) +
    geom_point(size = 2) +
    theme_bw() +
...
p_pca <- ggplot(p_data, aes(x=PC1, y=PC2, color = Water_body)) +
    geom_point(size = 2) +
...
adn <- adonis2(brayCurt~Water_body, data=testMeta, by="margin")
print(adn)
...
brayCurt_long[,Water_body := c("within","between")[as.integer(meta[Sample1, "Water_body"] != meta[Sample2, "Water_body"])+1]]
brayCurt_long[,Water_body := factor(Water_body, levels = c("within","between"))]
Water_body_long <- paste(meta[brayCurt_long[,Sample1],"Water_body"],meta[brayCurt_long[,Sample2], "Water_body"], sep = "-")
n <- combn(unique(meta[["Water_body"]]),2)
crct <- apply(n,2,paste,collapse="-")
wrng <- apply(as.matrix(n[c(2,1),]),2,paste,collapse="-")
for(i in 1:ncol(n)){
    Water_body_long <- gsub(wrng[i],crct[i],Water_body_long)
}
brayCurt_long[,Water_body_long:=Water_body_long]


p_boxDist <- ggplot(data = brayCurt_long, aes(x=Water_body_long, y=Bray_Curtis)) +
    geom_boxplot() +
    ylab("Distance") +
    theme_bw() +
    xlab("") +
...
p_alpha <- ggplot(alphaDiv, aes(x=Water_body, y=value, fill = Water_body)) +
    geom_boxplot() +
    facet_wrap(variable~., scale ="free_y",ncol=1) +
    theme_bw() +
    ylab("Value")+
    labs(fill = "Water body type", x = "Water body type")+
    theme(axis.text.x = element_text(angle = 45, hjust=1, vjust=1))
p_alpha
...
ggsave(p_all,file = "figures/FigS10_WaterBodyOnlyOli.pdf", width = 8, height= 8)
...
p_toprow <- cowplot::plot_grid(p_umap,p_pca,p_boxDist,ncol=3,labels=LETTERS[1:3], rel_widths=c(2,2,1))
p_toprow <- cowplot::plot_grid(p_umap,p_pca,ncol=2,labels=LETTERS[1:2], rel_widths=c(2,2,1))
p_botrow <- cowplot::plot_grid(p_boxDist,p_alpha, ncol = 2,labels=LETTERS[3:4], rel_widths = c(1,3))
p_all <- cowplot::plot_grid(p_toprow, p_alpha,ncol=1,labels=c("","D"), rel_heights = c(1,1))
p_all <- cowplot::plot_grid(p_toprow, p_botrow,ncol=1, rel_heights = c(1,1.3))
p_all

ggsave(p_all,file = "figures/FigS9_SiteIDOnlyOli.pdf", width = 8, height= 8)
...
ggsave(p_all,file = "figures/FigS11_NutrientLoadOliOnly.pdf", width = 8, height= 8)
...
p_umap <- ggplot(p_data, aes(x=UMAP1, y=UMAP2, color = Reproduction)) +
    geom_point(size = 2) +
    theme_bw() +
    theme(legend.position = "none")
p_umap
...
p_pca <- ggplot(p_data, aes(x=PC1, y=PC2, color = Reproduction)) +
    geom_point(size = 2) +
    ylab(paste0("PC2 (",round(pca_var[2]*100,2),"%)")) +
    xlab(paste0("PC1 (",round(pca_var[1]*100,2),"%)")) +
    theme_bw() +
...
adn <- adonis2(brayCurt~Reproduction, data=testMeta, by="margin")
print(adn)
...
# add the information if within or between population IDs
brayCurt_long[,Reproduction := c("within","between")[as.integer(meta[Sample1, "Reproduction"] != meta[Sample2, "Reproduction"])+1]]
brayCurt_long[,Reproduction := factor(Reproduction, levels = c("within","between"))]
Reproduction_long <- paste(meta[brayCurt_long[,Sample1],"Reproduction"],meta[brayCurt_long[,Sample2], "Reproduction"], sep = "-")
n <- combn(unique(meta[["Reproduction"]]),2)
crct <- apply(n,2,paste,collapse="-")
wrng <- apply(n[c(2,1),],2,paste,collapse="-")
for(i in 1:ncol(n)){
    Reproduction_long <- gsub(wrng[i],crct[i],Reproduction_long)
}
brayCurt_long[,Reproduction_long:=Reproduction_long]
...
p_alpha <- ggplot(alphaDiv, aes(x=Reproduction, y=value, fill = Reproduction)) +
    geom_boxplot() +
    facet_wrap(variable~., scale ="free_y",ncol=1) +
    theme_bw() +
    ylab("Value")+
...
ggsave(p_all,file = "figures/FigS12_Reproduction.pdf", width = 8, height= 8)
